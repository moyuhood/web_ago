  ...
  useEffect(() => {
    ...
    if (platform === 'web') {
      ...
      const screenShareToken = '';
        var iframe = document.getElementById("3dviewer") as HTMLIFrameElement;;
        if (iframe == null) return;
        var iWindow = iframe.contentWindow;
        if (iWindow == null) return;
        var player = iWindow.document.getElementById("player");
        if (player == null) return;
        var canvasDOM = player.lastChild as CanvasElement;
        if (canvasDOM == null) return;
        const canvasStream = canvasDOM.captureStream(60);
        
        webClient.start3dViewerShare(screenShareToken, canvasStream).then(() => {
...
const items = [
  ...
  const showTools = drawable === 'drawable';
    const showIframe = false ? { display: 'hidden' } : {};

  return (
      <div className={`media-board ${drawable}`}>
          {me.role === 'teacher' && room ?
          <iframe className ={`frame_3d`} id="3dviewer" src="./newhouse/index.html" frameBorder="no"  ></iframe>
